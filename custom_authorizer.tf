resource "aws_api_gateway_authorizer" "rest_authorizer" {
  ...
  type                           = coalesce(try(each.value.query_param, "-"), "-") != "-" ? "REQUEST" : "TOKEN"
  identity_source                = coalesce(try(each.value.query_param, "-"), "-") != "-" ? try("method.request.querystring.${each.value.query_param}", "method.request.header.Authorization") : "method.request.header.Authorization"
  ...
  identity_validation_expression = coalesce(try(each.value.query_param, "-"), "-") != "-" ? null : "^Bearer [-0-9a-zA-z\\.]*$"
}
