resource "aws_api_gateway_authorizer" "rest_authorizer" {
  ...
  identity_source                = try(each.value.query_param, "") != "" ? try("method.request.querystring.${each.value.query_param}", "method.request.header.Authorization") : "method.request.header.Authorization"
